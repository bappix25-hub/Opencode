...
def score_pattern_match(snapshot: dict) -> tuple:
    data = load_data()
    golden = data.get("golden_snapshots", [])
    if not golden:
        return 0.0, "কোনো গোল্ডেন প্যাটার্ন নেই"
    ...
    for g in golden:
        ...
        g_liq = g.get("initial_liquidity", 0)
        s_liq = snapshot.get("initial_liquidity", 0)
        if g_liq > 0 and s_liq > 0:
            if s_liq <= g_liq * 2.0:
                s += 0.20
                reasons.append("liq_ok")

        g_b30 = g.get("buyers_30s", 0)
        s_b30 = snapshot.get("buyers_30s", 0)
        if g_b30 > 0 and s_b30 > 0:
            if s_b30 >= g_b30 * 0.5:
                s += 0.25
                reasons.append(f"buyers30s={s_b30}")

        g_v60 = g.get("velocity_60s", 0)
        s_v60 = snapshot.get("velocity_60s", 0)
        if g_v60 > 0 and s_v60 > 0:
            if s_v60 >= g_v60 * 0.5:
                s += 0.20
                reasons.append(f"vel60s={s_v60:.1f}")

        g_c300 = g.get("curve_300s", 0)
        s_c300 = snapshot.get("curve_300s", 0)
        if g_c300 > 0 and s_c300 > 0:
            if s_c300 >= g_c300 * 0.5:
                s += 0.15
                reasons.append(f"curve300={s_c300:.0f}%")

        g_u300 = g.get("unique_300s", 0)
        s_u300 = snapshot.get("unique_300s", 0)
        if g_u300 > 0 and s_u300 > 0:
            if s_u300 >= g_u300 * 0.5:
                s += 0.10
                reasons.append(f"wallets={s_u300}")

        g_vol = g.get("volume_300s", 0)
        s_vol = snapshot.get("volume_300s", 0)
        if g_vol > 0 and s_vol > 0:
            if s_vol >= g_vol * 0.3:
                s += 0.10
                reasons.append(f"vol={s_vol:.1f}")

        scores.append((s, "+".join(reasons) if reasons else "no_match"))

    if not scores:
        return 0.0, "স্কোরিং সম্ভব নয়"

    best_score, best_reason = max(scores, key=lambda x: x[0])
    return min(best_score, 1.0), f"best={best_score:.2f} ({best_reason})"
