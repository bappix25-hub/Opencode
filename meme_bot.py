...
class MemeBot:
    ...
    async def start(self):
        ...
        try:
            self.state = self.state.load_from_data_file()
            logger.info(f"♻️ BotState লোড: launch={len(self.state.launch_tracking)} tracked={len(self.state.tracked_coins)} signals={len(self.state.signal_tracking)}")
        except Exception as e:
            logger.debug(f"BotState load error: {e}")
    ...
    async def shutdown(self):
        ...
        try:
            await self.state.save_to_data_file()
            logger.info("💾 BotState saved on shutdown")
        except Exception as e:
            logger.error(f"BotState save error on shutdown: {e}")
        ...
        while True:
            try:
                ...
                try:
                    ...
                    logger.debug(f"save bot_state error: {e}")
